...
const GameView = ({ viewArea, puzzleArea, isSolving }) => {
  const orientation =
    Math.min(viewArea.w, viewArea.h) === viewArea.h ? "landscape" : "portrait";
  console.log(" puzzle area? " + puzzleArea.w);

  let puzzleScale = 1;

  let availableGameWidth = viewArea.w;
  let availableGameHeight =
    orientation === "landscape" && puzzleArea.h < viewArea.h
      ? puzzleArea.h
      : viewArea.h;
  ...
  if (orientation === "landscape") {
    ...
    if (puzzleArea.w * scaleForHeight > availableGameWidth - 310) {
      puzzleScale =
        (puzzleScale * (availableGameWidth - 310)) /
        (puzzleArea.w * scaleForHeight);
    }
    ...
    if (puzzleArea.h * scaleForWidth > availableGameHeight - 260) {
      puzzleScale =
        (puzzleScale * (availableGameHeight - 260)) /
        (puzzleArea.h * scaleForWidth);
    }
  ...
  const leftpos =
    orientation === "landscape"
      ? (availablePuzzleWidth - puzzleArea.w * puzzleScale) / 2
      : (availablePuzzleWidth - puzzleArea.w * puzzleScale) / 2 - 15; //2 for border width
  ...
  console.log(
    " left percent? " +
      ...
      ", left pos: " +
      leftpos +
      ", available puzzle width: " +
      availablePuzzleWidth +
      ", available game width: " +
      availableGameWidth
  );
  const toppos =
    orientation === "landscape"
      ? (availablePuzzleHeight - puzzleArea.h * puzzleScale) / 2 - 15
      : (availablePuzzleHeight - puzzleArea.h * puzzleScale) / 2;
      ...
      <div className="wholepage">
        ...
        <div className="writeup">
          <p className="pagedescription">
            A classic 8 or 15 puzzle that I built with React that has
            interactive sliding tiles. The ai solver uses the{" "}
            <a href="https://en.wikipedia.org/wiki/Fringe_search">
              fringe search{" "}
            </a>
            algorithm and manhattan distance heuristic, which is sufficient for
            the 8 piece puzzle. For the 15 piece a non-optimal but short
            solution is found by breaking up the problem and stringing together
            those solutions, an idea based on{" "}
            <a href="https://pdfs.semanticscholar.org/21be/9f73ab7afb7991b8cfbdaf96e4124a0bec89.pdf">
              this
            </a>{" "}
            paper by Ian Parberry.
          </p>
          <p className="pagedescription">
            While most configurations of the 15 puzzle are solved within a few
            seconds, the hardest can take up from 30 to 50 seconds. For this
            reason the solution is carried out in a web worker so the browser
            doesn't lock up. This puzzle works great on mobile devices due to
            the responsive design of the layout and memory efficient ai.
          </p>
        </div>
      </div>
    );
